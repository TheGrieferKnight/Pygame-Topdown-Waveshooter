...
def main():
    ...
    # Add stat buttons and labels
    for stat_name in ["Bullet Damage", "Max Health", "Speed"]:
        stat_label = stat_menu.add.label(
            "{}: {}".format(
                stat_name,
                getattr(main_game.player_statss, stat_name.replace(" ", "_").lower()),
            )
        )
        stat_button = stat_menu.add.button(
            "Upgrade {} ({} points)".format(
                stat_name, main_game.player_statss.get_upgrade_cost(stat_name)
            ),
            lambda name=stat_name: upgrade_stat(name),
        )
    ...
    engine.set_sound(
        sound.SOUND_TYPE_WIDGET_SELECTION, "assets/enviroment/menu_click.mp3"
    )
    engine.set_sound(sound.SOUND_TYPE_OPEN_MENU, "assets/enviroment/menu_click.mp3")
